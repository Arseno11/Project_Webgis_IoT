    ...
    <div id="main">
      ...
      function updateChart() {
        fetch('ambildata.php')
          ...
          .then(data => {
            // Loop through the data and update chart for each id_alat
            data.results.forEach(result => {
              var id_alat = result.id_alat;
              var jarak = result.jarak;
              var hujan = result.hujan;
              var waktu = result.waktu;
              // Check if chart data for this id_alat already exists
              if (!chartData[id_alat]) {
                // Create new dataset for this id_alat
                chartData[id_alat] = {
                  label: 'ID Alat ' + id_alat,
                  data: [],
                  backgroundColor: 'rgba(0, 0, 0, 0)',
                  borderColor: getRandomColor(),
                  borderWidth: 1
                };
                myChart.data.datasets.push(chartData[id_alat]);
              }
              // Update chart data for this id_alat
              chartData[id_alat].data.push({
                x: waktu,
                y: jarak
              });
              chartData[id_alat].data.push({
            ...
            Object.keys(chartData).forEach(id_alat => {
              chartData[id_alat].data.sort((a, b) => a.x - b.x);
            });
