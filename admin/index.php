        ...
        fetch('ambildata.php')
          ...
          .then(data => {
            ...
            if (myChart.data.labels.length > 0) {
              lastUpdateTime = myChart.data.labels[myChart.data.labels.length - 1];
            }

            // Only update chart if new data is available
            if (data.results.length > 0) {
              // Check if any data is missing
              for (var i = 0; i < data.results.length - 1; i++) {
                if (parseInt(data.results[i].id_alat) + 1 != parseInt(data.results[i + 1].id_alat)) {
                  // Data is missing, insert empty data
                  myChart.data.labels.push('');
                  myChart.data.datasets[0].data.push(null);
                }
              }

              // Add new data to chart
              for (var i = 0; i < data.results.length; i++) {
                if (parseInt(data.results[i].waktu) > lastUpdateTime) {
                  myChart.data.labels.push(data.results[i].waktu);
                  myChart.data.datasets[0].data.push(data.results[i].jarak);
                }
              }
