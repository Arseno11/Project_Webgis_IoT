        ...
        fetch('ambildata.php')
          ...
          .then(data => {
            ...
            data.results.forEach((result) => {
              ...
              groupedData[result.id_alat].labels.push(result.waktu);
              groupedData[result.id_alat].data.jarak.push(result.jarak);
              groupedData[result.id_alat].data.hujan.push(result.hujan);
              // Change color of the line based on the value of the data point
              if (result.jarak < 10) {
                groupedData[result.id_alat].color = 'rgba(255, 0, 0, 1)';
              } else if (result.jarak > 10 && result.jarak <= 20) {
                groupedData[result.id_alat].color = 'rgba(255, 255, 0, 1)';
              } else {
                groupedData[result.id_alat].color = 'rgba(16, 255, 79, 1)';
              }
            });
            ...
            Object.keys(groupedData).forEach((key) => {
              myChart.data.labels = groupedData[key].labels;
              myChart.data.datasets.push({
                label: 'Jarak',
                data: groupedData[key].data.jarak,
                borderColor: groupedData[key].color,
                borderWidth: 1
              });
              myChart.data.datasets.push({
                label: 'Hujan',
                data: groupedData[key].data.hujan,
                borderColor: groupedData[key].color,
                borderWidth: 1,
                borderDash: [5, 5]
              });
