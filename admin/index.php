            ...
            for (let i = 0; i < myChart.data.datasets.length; i++) {
              ...
              for (let j = 0; j < dataset.data.length - 1; j++) {
                ...
                if (timeDiff > 5) {
                  ...
                  } else if (dataset.data[j].y < 20) {
                    dataset.borderColor = 'rgba(255, 165, 0, 1)';
                    dataset.backgroundColor = 'rgba(255, 190, 132, 0.2)';
                  } else {
                    dataset.borderColor = 'rgba(0, 128, 0, 1)';
                    dataset.backgroundColor = 'rgba(102, 255, 102, 0.2)';
                  }
                }
                // Hide or reduce width of older data for each dataset
                if (j < dataset.data.length - newDataLength) {
                  dataset.borderWidth = 0;
                  dataset.borderColor = 'rgba(0, 0, 0, 0)';
                  dataset.backgroundColor = 'rgba(0, 0, 0, 0)';
                } else if (j === dataset.data.length - newDataLength) {
                  dataset.borderWidth = 1;
                  dataset.borderDash = [];
                  // Change color of the line based on the value of the data point
                  if (dataset.data[j].y < 10) {
